...
for mail in reversed(last_email):
    ...
    email_from = str(email_message['From'])     # 보낸 사람
    email_date = str(email_message['Date'])     # 받은 시간
    subject, encode = find_encoding_info(email_message['Subject'])  # 제목
    subject_str = str(subject)      # 문자열 형태로 바인딩
